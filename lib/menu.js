...
/** Merge admin price overrides onto a catalog item (client-safe, no filesystem). */
export function applyMenuPriceOverrides(item, overrides = {}) {
  if (!item || !overrides || typeof overrides !== "object") return item
  const next = { ...item }
  if (overrides.small !== undefined && item.small !== undefined) {
    next.small = Number(overrides.small)
  }
  if (overrides.regular !== undefined) {
    next.regular = Number(overrides.regular)
  }
  return next
}
